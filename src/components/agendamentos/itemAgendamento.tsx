...
export const ItemAgendamento = ({data}: Props) => {
    ...
    return (
        <div className={`bg-[#F4F4F5] dark:bg-[#18181B] flex justify-between items-center rounded-2xl py-3 px-5 md:px-10 border ${data.status === "Feito" && "border-green-600"}  ${data.status === "Cancelado" && "border-red-500"} ${hoje > Number(dia) && "border-red-500"} ${hoje < Number(dia) && data.status === "Confirmado" && "border-yellow-500"}  ${hoje === Number(dia) ? "border-blue-500 border-2" : "border-gray-700"}`}>
